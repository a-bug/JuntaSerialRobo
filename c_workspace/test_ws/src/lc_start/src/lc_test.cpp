...
enum Transition { CONFIGURE = 1 , CLEANUP, ACTIVATE, DEACTIVATE, UNCONFIGURED_SHUTDOWN }

// which node to handle
static constexpr char const * lifecycle_node = "serial_bridge_node";
...
// serial_bridge_node/get_state
// serial_bridge_node/change_state
static constexpr char const * node_get_state_topic = "serial_bridge_node/get_state";
static constexpr char const * node_change_state_topic = "serial_bridge_node/change_state";
...
public:
  ...
  get_state(std::chrono::seconds time_out = 3s)
  {
    ...
    // state of the serial_bridge_node node.
...
*/
void
callee_script(std::shared_ptr<LifecycleServiceClient> lc_client, Transition T)
{
  lifecycle_msgs trans = ('lifecycle_msgs::msg::Transition::TRANSITION_' + T); 

  {
    if (!lc_client->change_state(trans)) {
      return;
    }
    if (!lc_client->get_state()) {
      return;
